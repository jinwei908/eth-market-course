...
const createFormState = ({price, email, confirmationEmail}, agreedTOS, isNewPurchase)=>{
    ...
    if(isNewPurchase){
        if (confirmationEmail != email){
            return _createFormState(true, "Confirmation Email not equal to Email");
        } else if (confirmationEmail.length === 0 || email.length === 0){
            return _createFormState(true, "Email or Confirmation Email should not be empty");
        }
    }
    if (!agreedTOS){
        return _createFormState(true, "You need to agree to the terms of service in order to submit the form.");
    }
...
export default function OrderModal({course, onClose, onSubmit, isNewPurchase}){
    ...
    useEffect(()=>{
      ...
      setOrder({
          ...defaultOrder,
          price: eth.perItem //the data for eth
      })
    }, [course])
    ...
    const formState = createFormState(order, agreedTOS, isNewPurchase); //why does this always update? WHat is the magic here
